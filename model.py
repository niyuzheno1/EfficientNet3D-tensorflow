...
class MBConvBlock3D(tf.keras.layers.Layer):
    """
    ...
    def __init__(self, block_args, global_params):
        ...
        if self._block_args.expand_ratio != 1:
            ...
            self._bn0  = tf.keras.layers.BatchNormalization(momentum=self._bn_mom, epsilon=self._bn_eps)
        ...
        self._bn1  = tf.keras.layers.BatchNormalization(momentum=self._bn_mom, epsilon=self._bn_eps)
        ...
        self._bn2  = tf.keras.layers.BatchNormalization(momentum=self._bn_mom, epsilon=self._bn_eps)
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self.has_se:
            ...
            x_squeezed = tfa.layers.AdaptiveAvgPooling3D(1)(x)
            x_squeezed = self._se_expand(self._swish(self._se_reduce(x_squeezed)))
            # x = torch.sigmoid(x_squeezed) * x
            x = tf.keras.layers.multiply()([x, tf.keras.activations.sigmoid(x_squeezed)])
...
class EfficientNet3D(tf.keras.Model):
    """
    ...
    def __init__(self, blocks_args=None, global_params=None, in_channels=3):
        ...
        self._bn0  = tf.keras.layers.BatchNormalization(momentum=bn_mom, epsilon=bn_eps)
        ...
        for block_args in self._blocks_args:
            ...
            self._blocks.add(MBConvBlock3D(block_args, self._global_params))
        ...
        self._bn1  = tf.keras.layers.BatchNormalization(momentum=bn_mom, epsilon=bn_eps)

        # Final linear layer
        #self._avg_pooling = nn.AdaptiveAvgPool3d(1)
        self._avg_pooling = tf.keras.layers.AdaptiveAvgPooling3D(1)
        # self._dropout = nn.Dropout(self._global_params.dropout_rate)
        self._dropout = tf.keras.layers.Dropout(self._global_params.dropout_rate)
        # self._fc = nn.Linear(out_channels, self._global_params.num_classes)
        self._fc = tf.keras.layers.Dense(self._global_params.num_classes)
